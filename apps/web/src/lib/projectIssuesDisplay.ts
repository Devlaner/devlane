...
export function parseProjectIssuesDisplay(raw: string | null): ProjectIssuesDisplayState | null {
  ...
  try {
    ...
    const parsedSubGroupBy = GROUP_BY_OPTIONS.includes(p.subGroupBy as SavedViewGroupBy)
      ? (p.subGroupBy as SavedViewGroupBy)
      : DEFAULT_PROJECT_ISSUES_DISPLAY.subGroupBy;
    const subGroupBy = parsedSubGroupBy === groupBy ? 'none' : parsedSubGroupBy;
...
export function serializeProjectIssuesDisplay(s: ProjectIssuesDisplayState): string {
  return JSON.stringify({
    ...
    groupBy: s.groupBy,
    subGroupBy: s.subGroupBy === s.groupBy ? 'none' : s.subGroupBy,
    orderBy: s.orderBy,
...
export function fromDisplayPayload(p: ProjectIssuesDisplayPayload): ProjectIssuesDisplayState {
  ...
  return {
    ...
    groupBy: GROUP_BY_OPTIONS.includes(p.groupBy)
      ...
      : DEFAULT_PROJECT_ISSUES_DISPLAY.groupBy,
    subGroupBy:
      GROUP_BY_OPTIONS.includes(p.subGroupBy) && p.subGroupBy !== p.groupBy
        ? p.subGroupBy
        : DEFAULT_PROJECT_ISSUES_DISPLAY.subGroupBy,
