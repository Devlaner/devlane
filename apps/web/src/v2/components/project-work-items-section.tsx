...
export interface ProjectWorkItemsSectionProps {
  workspaceSlug: string;
  projectId: string;
  workspace: WorkspaceApiResponse;
  project: ProjectApiResponse;
  /** The list itself: rows, filters, display, selection, mutations. */
  controller: ReturnType<typeof useProjectIssuesController>;
  layout: string;
  onLayoutChange: (next: string) => void;
  /** Buttons added to the right of the toolbar, before the create button. */
  toolbarExtras?: ReactNode;
  /** CSV import belongs to the project list, not to a single cycle. */
  showImportCsv?: boolean;
  /** Pre-fills the create dialog, e.g. with the cycle being viewed. */
  createInitialValues?: WorkItemInitialValues;
  /** Copy for the "nothing here yet" state; the project list's by default. */
  emptyTitle?: string;
  emptyDescription?: string;
}

/**
 * A project's work item list: toolbar, the five layouts, selection, bulk
 * actions and creation — everything the list is, apart from what page it sits
 * on and what heading stands above it.
 *
 * The cycle page is this same list narrowed to one cycle. Rendering it from
 * here rather than rebuilding it there is what keeps a cycle's rows, columns,
 * inline editors, grouping and shortcuts identical to the project list's.
 */
      ...
      {issues.length === 0 && (
        <Empty className="rounded-xl border border-dashed">
          <EmptyHeader>
            ...
            <EmptyTitle>
              {emptyTitle ?? t('issues.emptyTitle', 'Create your first work item')}
            </EmptyTitle>
            <EmptyDescription>
              {emptyDescription ??
                t(
