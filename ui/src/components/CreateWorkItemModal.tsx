...
export function CreateWorkItemModal({
  ...
  const assigneeNames =
    assigneeIds
      ...
      .filter(Boolean)
      .join(', ') || '';
  const labelNames = labelIds.map((id) => labels.find((l) => l.id === id)?.name).filter(Boolean).join(', ') || '';
  ...
  const filteredUsers = members.filter((u) => q(u.name).includes(q(assigneeSearch)) || q(u.id).includes(q(assigneeSearch)));
    ...
    <>
      ...
      >
        <div className="px-5 pt-5 pb-2">
          <h2 id="create-work-item-title" className="text-xl font-bold text-[var(--txt-primary)]">
        ...
        <div className="px-5 py-4">
          ...
          <div className="flex flex-nowrap items-center gap-1.5 overflow-x-auto pb-1">
              ...
              <div className="overflow-auto py-0.5 [&_button]:px-2 [&_button]:py-1 [&_button]:text-xs">
                {filteredStates.map((s) => (
                  <button
                    key={s.id}
                    type="button"
                    onClick={() => { setStateId(s.id); setOpenDropdown(null); }}
                    className="w-full text-left text-[var(--txt-primary)] hover:bg-[var(--bg-layer-1-hover)]"
                  >
                    {s.name}
                  </button>
            ...
            <Dropdown
              ...
              displayValue={priority === 'none' ? 'None' : priority.charAt(0).toUpperCase() + priority.slice(1)}
              compact
            ...
            >
              ...
              <div className="overflow-auto py-0.5 [&_button]:px-2 [&_button]:py-1 [&_button]:text-xs">
                <button
                  type="button"
                  onClick={() => { setAssigneeIds([]); setOpenDropdown(null); }}
                ...
                {filteredUsers.map((u) => (
                  <button
                    key={u.id}
                    type="button"
                    onClick={() => setAssigneeIds((prev) => (prev.includes(u.id) ? prev.filter((x) => x !== u.id) : [...prev, u.id]))}
                    className="w-full text-left text-[var(--txt-primary)] hover:bg-[var(--bg-layer-1-hover)]"
                  >
            ...
            >
              ...
              <div className="overflow-auto py-0.5 [&_button]:px-2 [&_button]:py-1 [&_button]:text-xs">
                <button
                  type="button"
                  onClick={() => { setCycleId(null); setOpenDropdown(null); }}
                  className="w-full text-left text-[var(--txt-tertiary)] hover:bg-[var(--bg-layer-1-hover)]"
                >
                  No cycle
                </button>
                {filteredCycles.map((c) => (
                  <button
                    key={c.id}
                    type="button"
                    onClick={() => { setCycleId(c.id); setOpenDropdown(null); }}
                    className="w-full text-left text-[var(--txt-primary)] hover:bg-[var(--bg-layer-1-hover)]"
                  >
                    {c.name}
                  </button>
            ...
            >
              ...
              <div className="overflow-auto py-0.5 [&_button]:px-2 [&_button]:py-1 [&_button]:text-xs">
                <button
                  type="button"
                  onClick={() => { setModuleId(null); setOpenDropdown(null); }}
                  className="w-full text-left text-[var(--txt-tertiary)] hover:bg-[var(--bg-layer-1-hover)]"
                >
                  No module
                </button>
                {filteredModules.map((m) => (
                  <button
                    key={m.id}
                    type="button"
                    onClick={() => { setModuleId(m.id); setOpenDropdown(null); }}
                    className="w-full text-left text-[var(--txt-primary)] hover:bg-[var(--bg-layer-1-hover)]"
                  >
                    {m.name}
                  </button>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border-subtle)] px-5 py-4">
          <div className="flex flex-col gap-1">
            ...
            {createError && (
              <p className="text-sm text-[var(--txt-danger-primary)]">{createError}</p>
            )}
          </div>
