...
const ICON_COLORS = [
  '#94a3b8', '#64748b', '#6366f1', '#3b82f6', '#0ea5e9', '#22c55e', '#eab308', '#f97316', '#ec4899', '#ffffff',
];
...
export function ProjectIconDisplay({
  ...
  if (emoji) {
    return <span className={className} style={{ fontSize: size }} role="img" aria-hidden>{emoji}</span>;
  }
...
export function ProjectIconModal({
  ...
  const filteredEmojis = useMemo(() => {
    ...
    return EMOJI_LIST.filter((e) => e.includes(q) || String.fromCodePoint(e.codePointAt(0)!).toLowerCase().includes(q));
  }, [emojiSearch]);
  ...
  const footer = (
    <button type="button" onClick={onClose} className="rounded-[var(--radius-md)] border border-[var(--border-subtle)] bg-[var(--bg-layer-2)] px-3 py-1.5 text-sm font-medium text-[var(--txt-primary)] hover:bg-[var(--bg-layer-2-hover)]">
      Cancel
    </button>
  );
      ...
      <div className="flex gap-2 border-b border-[var(--border-subtle)] pb-3 mb-3">
        <button
          ...
          className={`rounded-[var(--radius-md)] px-3 py-1.5 text-sm font-medium transition-colors ${
            tab === TAB_EMOJI ? 'bg-[var(--brand-default)] text-white' : 'text-[var(--txt-secondary)] hover:bg-[var(--bg-layer-transparent-hover)]'
          }`}
        >
          Emoji
        </button>
        <button
          ...
          className={`rounded-[var(--radius-md)] px-3 py-1.5 text-sm font-medium transition-colors ${
            tab === TAB_ICON ? 'bg-[var(--brand-default)] text-white' : 'text-[var(--txt-secondary)] hover:bg-[var(--bg-layer-transparent-hover)]'
          }`}
        >
          Icon
        </button>
