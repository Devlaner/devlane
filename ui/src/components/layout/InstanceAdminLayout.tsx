...
const SECTIONS = [
  { path: 'general', label: 'General', desc: 'Identify your instances and get key details.', Icon: IconSettings },
  { path: 'workspace', label: 'Workspaces', desc: 'Manage all workspaces on this instance.', Icon: IconBriefcase },
  { path: 'email', label: 'Email', desc: 'Configure your SMTP controls.', Icon: IconEnvelope },
  { path: 'authentication', label: 'Authentication', desc: 'Configure authentication modes.', Icon: IconLock },
  { path: 'ai', label: 'Artificial intelligence', desc: 'Configure your OpenAI creds.', Icon: IconCpu },
  { path: 'image', label: 'Images in Devlane', desc: 'Allow third-party image libraries.', Icon: IconImage },
] as const;
        ...
        <aside className="flex w-72 shrink-0 flex-col border-r border-[var(--border-subtle)] bg-[var(--bg-surface-1)]">
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1 overflow-hidden">
                    ...
                    <div className="truncate text-[10px] leading-tight text-[var(--txt-tertiary)]">{desc}</div>
                  </div>
                </NavLink>
              );
            })}
          ...
          <div className="flex shrink-0 flex-col gap-0.5 border-t border-[var(--border-subtle)] p-1.5">
            ...
            <div className="flex gap-0.5">
              <button type="button" className="flex size-7 shrink-0 items-center justify-center rounded-md text-[var(--txt-icon-tertiary)] hover:bg-[var(--bg-layer-1-hover)] hover:text-[var(--txt-icon-secondary)] [&_svg]:size-4" aria-label="Help">
                <IconHelp />
              </button>
              <Link to="/acme" className="flex size-7 shrink-0 items-center justify-center rounded-md text-[var(--txt-icon-tertiary)] hover:bg-[var(--bg-layer-1-hover)] hover:text-[var(--txt-icon-secondary)] [&_svg]:size-4" aria-label="Back">
                <IconArrowLeft />
              </Link>
            </div>
          </div>
        ...
        <main className="min-w-0 flex-1 overflow-auto p-5">
          <div className="mb-4 flex items-center gap-1.5 text-xs text-[var(--txt-secondary)] [&_svg]:size-3.5">
            ...
            {breadcrumbTail && (
              <>
                <span className="text-[var(--txt-icon-tertiary)]">&gt;</span>
                <span className="text-[var(--txt-primary)]">{breadcrumbTail}</span>
              </>
            )}
          </div>
          <Outlet />
