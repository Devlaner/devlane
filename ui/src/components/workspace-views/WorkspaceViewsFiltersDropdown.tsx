...
const IconNone = () => (
  <span className="flex size-4 items-center justify-center text-[10px] text-[var(--txt-icon-tertiary)]">—</span>
);
...
const IconCompleted = () => (
  <span className="flex size-4 items-center justify-center rounded-full bg-green-500 text-white text-[10px]">✓</span>
);
const IconCanceled = () => (
  <span className="flex size-4 items-center justify-center rounded-full border border-[var(--border-subtle)] text-[10px] text-[var(--txt-tertiary)]">✕</span>
);
...
export function WorkspaceViewsFiltersDropdown({
  ...
  const [statesByProject, setStatesByProject] = useState<Record<string, StateApiResponse[]>>({});
      ...
      .then((proj) => {
        ...
        return Promise.all(
          proj.map((p) =>
            ...
            ]).then(([s, l]) => ({ projectId: p.id, states: s ?? [], labels: l ?? [] }))
          )
        );
      })
      .then((results) => {
  ...
  const updateFilters = useCallback(
    (updater: (prev: WorkspaceViewFilters) => WorkspaceViewFilters) => {
      setSearchParams((prev) => {
        ...
        const nextFilters = updater(parseWorkspaceViewFiltersFromSearchParams(prev));
        const params = workspaceViewFiltersToSearchParams(nextFilters);
        const filterKeys = ["priority", "state_group", "assignee", "created_by", "label", "project", "grouping", "start_date", "due_date", "start_after", "start_before", "due_after", "due_before"];
    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto py-1" style={LONG_LIST_PANEL_STYLE}>
        ...
        >
          {PRIORITIES.filter((p) => filterSearch(PRIORITY_LABELS[p])).map((p) => (
            ...
            >
              ...
              />
              <span className="flex size-4 shrink-0 items-center justify-center">{PRIORITY_ICONS[p]}</span>
          ...
          {STATE_GROUPS.filter((g) => filterSearch(STATE_GROUP_LABELS[g])).map((g) => (
            ...
            >
              <input
                ...
                className="rounded border-[var(--border-subtle)]"
              />
              <span className="flex size-4 shrink-0 items-center justify-center">{STATE_GROUP_ICONS[g]}</span>
        ...
        >
          ...
          {members
            .filter((m) => filterSearch(m.member_display_name ?? m.member_email ?? m.member_id))
            .map((m) => (
              <label
                key={m.id}
                className="flex cursor-pointer items-center gap-2 px-3 py-1.5 text-sm text-[var(--txt-primary)] hover:bg-[var(--bg-layer-1-hover)]"
              >
                ...
                <span className="truncate">{m.member_display_name ?? m.member_email ?? m.member_id}</span>
              </label>
            ))}
          <button
            type="button"
              ...
              .map((l) => (
                ...
                >
                  ...
                  <span
                    ...
                    style={{ backgroundColor: l.color ?? "var(--txt-icon-tertiary)" }}
                  />
        ...
        >
          {projects.filter((p) => filterSearch(p.name)).map((p) => (
            <label
              key={p.id}
              className="flex cursor-pointer items-center gap-2 px-3 py-1.5 text-sm text-[var(--txt-primary)] hover:bg-[var(--bg-layer-1-hover)]"
            >
          ...
          {GROUPING_OPTIONS.map((g) => (
            ...
            >
              <input
                ...
                onChange={() => updateFilters((prev) => ({ ...prev, grouping: g }))}
    ...
    <DateRangeModal
      ...
      onApply={(after, before) => {
        if (dateRangeModal === "start") {
          updateFilters((prev) => ({
            ...prev,
            startDate: prev.startDate.includes("custom") ? prev.startDate : [...prev.startDate, "custom"],
            startAfter: after,
            startBefore: before,
          }));
        } else {
