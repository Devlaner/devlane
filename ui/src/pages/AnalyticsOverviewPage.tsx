...
import type { WorkspaceApiResponse, ProjectApiResponse, IssueApiResponse, WorkspaceMemberApiResponse } from '../api/types';
...
export function AnalyticsOverviewPage() {
    ...
    workspaceService
      ...
      .then((res) => {
        ...
        if (!cancelled && projs?.length) {
          return Promise.all(projs.map((p) => issueService.list(workspaceSlug!, p.id, { limit: 200 })));
        }
        return [];
      })
      .then((issueArrays) => {
      ...
      .catch(() => {
        if (!cancelled) setWorkspace(null); setProjects([]); setIssues([]); setMembers([]);
      })
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <h2 className="text-lg font-semibold text-[var(--txt-primary)]">Overview</h2>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {summaryCards.map(({ label, value }) => (
          ...
          >
            <p className="text-xs font-medium text-[var(--txt-tertiary)]">{label}</p>
            <p className="mt-1 text-2xl font-semibold text-[var(--txt-primary)]">{value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[2fr_1fr]">
        ...
        <section className="min-w-0 rounded-md border border-[var(--border-subtle)] bg-[var(--bg-surface-1)] p-6">
          <h3 className="mb-4 text-base font-semibold text-[var(--txt-primary)]">Project Insights</h3>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-[3fr_2fr]">
            {/* Work Items - radar chart */}
            <div className="min-w-0">
              <h4 className="mb-3 text-sm font-semibold text-[var(--txt-primary)]">Work Items</h4>
              <div className="rounded-md bg-[var(--bg-surface-1)] p-4">
                <div className="h-[400px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <RechartsRadarChart data={radarData} margin={{ top: 16, right: 16, bottom: 16, left: 16 }}>
            ...
            {/* Summary of Projects */}
            <div className="min-w-0">
              <h4 className="mb-2 text-sm font-semibold text-[var(--txt-primary)]">Summary of Projects</h4>
              <p className="mb-1 text-sm font-semibold text-[var(--txt-primary)]">All Projects</p>
              <p className="mb-3 text-sm text-[var(--txt-tertiary)]">Trend on charts</p>
              <ul className="space-y-3 text-sm">
                {radarDimensions.map((d) => (
                  <li key={d.name} className="flex items-center justify-between gap-4 text-[var(--txt-secondary)]">
                    <span>{d.name}</span>
                    <span className="shrink-0 font-medium tabular-nums text-[var(--txt-primary)]">{d.value}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <section className="min-w-0">
          <h3 className="mb-4 text-base font-semibold text-[var(--txt-primary)]">Active Projects</h3>
          <ul className="space-y-2">
            {projects.map((p) => (
              ...
              >
                ...
                <span className="min-w-0 flex-1 truncate font-medium text-[var(--txt-primary)]">{p.name}</span>
                <span className="shrink-0 rounded bg-[var(--bg-danger-subtle)] px-2 py-0.5 text-xs font-medium text-[var(--txt-danger-primary)]">0%</span>
              </li>
            ))}
          </ul>
        </section>
