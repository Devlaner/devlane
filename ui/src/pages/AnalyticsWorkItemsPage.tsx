...
export function AnalyticsWorkItemsPage() {
  ...
  useEffect(() => {
    ...
    workspaceService
      ...
      .catch(() => {
        if (!cancelled) setWorkspace(null); setProjects([]); setIssues([]); setStates([]);
      })
  ...
  const priorityCounts = issues.reduce<Record<string, number>>((acc, i) => {
    const p = !i.priority || i.priority === 'none' ? 'None' : i.priority.charAt(0).toUpperCase() + i.priority.slice(1);
    acc[p] = (acc[p] ?? 0) + 1;
    return acc;
  }, {});
  const priorityRows = Object.entries(priorityCounts).map(([priority, count]) => ({ priority, count }));
  ...
  const projectRows = projects.map((p) => {
    ...
    return {
      project: p,
      backlog: projIssues.filter((i) => getStateName(i.stateId) === 'Backlog').length,
      started: projIssues.filter((i) => getStateName(i.stateId) === 'In Progress').length,
      unstarted: projIssues.filter((i) => getStateName(i.stateId) === 'Todo').length,
      completed: projIssues.filter((i) => getStateName(i.stateId) === 'Done').length,
      cancelled: 0,
    };
  });
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <h2 className="text-lg font-semibold text-[var(--txt-primary)]">Work items</h2>

      {/* KPIs */}
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
        <div className="rounded-md border border-[var(--border-subtle)] bg-[var(--bg-surface-1)] px-4 py-3">
          <p className="text-xs font-medium text-[var(--txt-tertiary)]">Total Work items</p>
          <p className="mt-1 text-2xl font-semibold text-[var(--txt-primary)]">{issues.length}</p>
        </div>
        <div className="rounded-md border border-[var(--border-subtle)] bg-[var(--bg-surface-1)] px-4 py-3">
          <p className="text-xs font-medium text-[var(--txt-tertiary)]">Started Work items</p>
          <p className="mt-1 text-2xl font-semibold text-[var(--txt-primary)]">{startedCount}</p>
        </div>
        <div className="rounded-md border border-[var(--border-subtle)] bg-[var(--bg-surface-1)] px-4 py-3">
          <p className="text-xs font-medium text-[var(--txt-tertiary)]">Backlog Work items</p>
          <p className="mt-1 text-2xl font-semibold text-[var(--txt-primary)]">{backlogCount}</p>
        </div>
        <div className="rounded-md border border-[var(--border-subtle)] bg-[var(--bg-surface-1)] px-4 py-3">
          <p className="text-xs font-medium text-[var(--txt-tertiary)]">Unstarted Work items</p>
          <p className="mt-1 text-2xl font-semibold text-[var(--txt-primary)]">{unstartedCount}</p>
        </div>
        <div className="rounded-md border border-[var(--border-subtle)] bg-[var(--bg-surface-1)] px-4 py-3">
          <p className="text-xs font-medium text-[var(--txt-tertiary)]">Completed Work items</p>
          <p className="mt-1 text-2xl font-semibold text-[var(--txt-primary)]">{completedCount}</p>
        </div>
      </div>

      {/* Created vs Resolved */}
      <section>
        <h3 className="mb-4 text-base font-semibold text-[var(--txt-primary)]">Created vs Resolved</h3>
        <div className="rounded-md border border-[var(--border-subtle)] bg-[var(--bg-surface-1)] p-6">
          <div className="h-[280px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={createdResolvedData} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
                ...
                <XAxis
                  ...
                  label={{ value: 'DATE', position: 'insideBottom', offset: -4, fill: 'var(--txt-tertiary)', fontSize: 11 }}
                />
                <YAxis
                  tick={{ fill: 'var(--txt-secondary)', fontSize: 11 }}
                  tickLine={{ stroke: 'var(--border-subtle)' }}
                  axisLine={{ stroke: 'var(--border-subtle)' }}
                  label={{ value: 'NO. OF WORK ITEMS', angle: -90, position: 'insideLeft', fill: 'var(--txt-tertiary)', fontSize: 11 }}
                  domain={[0, 'auto']}
                  allowDecimals={false}
                />
                <Legend
                  ...
                  iconType="square"
                  iconSize={10}
                  formatter={(value) => <span className="text-xs text-[var(--txt-secondary)]">{value}</span>}
                />
                <Line
                  type="monotone"
                  dataKey="resolved"
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={priorityRows} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
                ...
                <XAxis
                  ...
                  tickLine={{ stroke: 'var(--border-subtle)' }}
                  axisLine={{ stroke: 'var(--border-subtle)' }}
                  label={{ value: 'PRIORITY', position: 'insideBottom', offset: -4, fill: 'var(--txt-tertiary)', fontSize: 11 }}
                />
                <YAxis
                  tick={{ fill: 'var(--txt-secondary)', fontSize: 11 }}
                  tickLine={{ stroke: 'var(--border-subtle)' }}
                  axisLine={{ stroke: 'var(--border-subtle)' }}
                  label={{ value: 'NO. OF WORK ITEM', angle: -90, position: 'insideLeft', fill: 'var(--txt-tertiary)', fontSize: 11 }}
                  domain={[0, 'auto']}
                  allowDecimals={false}
                />
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <h3 className="text-base font-semibold text-[var(--txt-primary)]">
              {priorityRows.length} Priority{priorityRows.length !== 1 ? 'ies' : ''}
            </h3>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-[var(--border-subtle)]">
                <th className="py-3 pr-4 font-medium text-[var(--txt-secondary)]">Priority</th>
                <th className="py-3 font-medium text-[var(--txt-secondary)]">Count</th>
              </tr>
            </thead>
            <tbody>
              {priorityRows.map(({ priority, count }) => (
                <tr key={priority} className="border-b border-[var(--border-subtle)] last:border-0">
                  <td className="py-3 pr-4 text-[var(--txt-primary)]">{priority}</td>
      ...
      <section>
        ...
        <div className="overflow-x-auto rounded-md border border-[var(--border-subtle)] bg-[var(--bg-surface-1)]">
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-[var(--border-subtle)]">
                <th className="py-3 pr-4 font-medium text-[var(--txt-secondary)]">Project</th>
                <th className="py-3 pr-4 font-medium text-[var(--txt-secondary)]">Backlog</th>
                <th className="py-3 pr-4 font-medium text-[var(--txt-secondary)]">Started</th>
                <th className="py-3 pr-4 font-medium text-[var(--txt-secondary)]">Unstarted</th>
                <th className="py-3 pr-4 font-medium text-[var(--txt-secondary)]">Completed</th>
                <th className="py-3 font-medium text-[var(--txt-secondary)]">Cancelled</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={project.id} className="border-b border-[var(--border-subtle)] last:border-0">
                  <td className="py-3 pr-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[var(--txt-primary)]">{project.name}</span>
                    </div>
                  </td>
                  <td className="py-3 pr-4 text-[var(--txt-secondary)]">{backlog}</td>
                  <td className="py-3 pr-4 text-[var(--txt-secondary)]">{started}</td>
                  <td className="py-3 pr-4 text-[var(--txt-secondary)]">{unstarted}</td>
                  <td className="py-3 pr-4 text-[var(--txt-secondary)]">{completed}</td>
                  <td className="py-3 text-[var(--txt-secondary)]">{cancelled}</td>
                </tr>
              ))}
            </tbody>
          </table>
