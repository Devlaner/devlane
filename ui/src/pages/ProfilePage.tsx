...
import type { WorkspaceApiResponse, ProjectApiResponse, IssueApiResponse, StateApiResponse, WorkspaceMemberApiResponse } from '../api/types';
...
export function ProfilePage() {
  ...
  useEffect(() => {
    ...
    workspaceService
      ...
      .then((w) => {
        ...
        return Promise.all([projectService.list(workspaceSlug), workspaceService.listMembers(workspaceSlug)]);
      })
      .then((res) => {
        const [list, mem] = res ?? [[], []];
      ...
      .catch(() => {
        if (!cancelled) setWorkspace(null); setProjects([]); setIssues([]); setStates([]); setMembers([]);
      })
  ...
  const workloadCategories = useMemo(() => {
    ...
    return [
      ...
      { id: 'not-started', label: 'Not started', color: '#3b82f6', stateIds: notStarted },
      { id: 'working-on', label: 'Working on', color: '#f59e0b', stateIds: workingOn },
      { id: 'completed', label: 'Completed', color: '#22c55e', stateIds: completed },
      { id: 'canceled', label: 'Canceled', color: '#ef4444', stateIds: canceled },
    ];
  }, [states]);

  const priorityCounts = useMemo(() => {
    const counts: Record<string, number> = { urgent: 0, high: 0, medium: 0, low: 0, none: 0 };
    issuesAssigned.forEach((i) => { counts[i.priority] = (counts[i.priority] ?? 0) + 1; });
    return counts;
  }, [issuesAssigned]);
  ...
  const recentActivity = useMemo((): Array<{ id: string; projectId: string; issueId: string; action: string; createdAt: string }> => {
  ...
  if (!workspace) {
    return (
      <div className="p-4 text-[var(--txt-secondary)]">Workspace not found.</div>
    );
  }
  ...
  return (
    <div className="flex h-full min-h-0 gap-6">
      ...
      <div className="min-w-0 flex-1 space-y-6 pb-8">
        ...
        {activeTab === 'summary' && (
          <>
            {/* Overview */}
            <section>
              <h2 className="mb-3 text-sm font-semibold text-[var(--txt-primary)]">Overview</h2>
                ...
                >
                  <Card variant="outlined" className="border-0 bg-transparent shadow-none">
                    <CardContent className="flex items-center gap-3 p-4">
                      ...
                      <div>
                        <p className="text-2xl font-semibold text-[var(--txt-primary)]">{issuesCreated.length}</p>
                        <p className="text-sm text-[var(--txt-secondary)]">Work items created</p>
                      </div>
                    </CardContent>
                  </Card>
                </button>
                ...
                >
                  <Card variant="outlined" className="border-0 bg-transparent shadow-none">
                    <CardContent className="flex items-center gap-3 p-4">
                      ...
                      <div>
                        <p className="text-2xl font-semibold text-[var(--txt-primary)]">{issuesAssigned.length}</p>
                        <p className="text-sm text-[var(--txt-secondary)]">Work items assigned</p>
                      </div>
                    </CardContent>
                  </Card>
                </button>
                ...
                >
                  <Card variant="outlined" className="border-0 bg-transparent shadow-none">
                    <CardContent className="flex items-center gap-3 p-4">
                      ...
                      <div>
                        <p className="text-2xl font-semibold text-[var(--txt-primary)]">{issuesSubscribed}</p>
                        <p className="text-sm text-[var(--txt-secondary)]">Work items subscribed</p>
                      </div>
                    </CardContent>
                  </Card>
                </button>
            ...
            {/* Workload */}
            <section>
              <h2 className="mb-3 text-sm font-semibold text-[var(--txt-primary)]">Workload</h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
                {workloadCategories.map((cat) => {
                  const count = cat.stateIds.reduce((sum, id) => sum + (workloadByState[id] ?? 0), 0);
                  return (
                    <Card key={cat.id} variant="outlined" className="border border-[var(--border-subtle)] bg-white">
                      <CardContent className="flex items-center gap-3 p-4">
                        <span
                        ...
                        />
                        <div>
                          <p className="text-2xl font-semibold text-[var(--txt-primary)]">{count}</p>
                          <p className="text-sm text-[var(--txt-secondary)]">{cat.label}</p>
                        </div>
                      </CardContent>
                    </Card>
                  );
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <section>
                <h2 className="mb-3 text-sm font-semibold text-[var(--txt-primary)]">Work items by Priority</h2>
                <Card variant="outlined" className="border border-[var(--border-subtle)] bg-white">
                  <CardContent className="p-4">
                    <div className="flex flex-col gap-2">
                      {(['urgent', 'high', 'medium', 'low', 'none'] as const).map((p) => {
                        ...
                        const barColor = p === 'urgent' || p === 'high' ? 'var(--bg-danger-subtle)' : p === 'medium' ? 'var(--bg-warning-subtle)' : 'var(--bg-layer-2)';
                        return (
                          <div key={p} className="flex items-center gap-3">
                            <span className="w-16 shrink-0 capitalize text-sm text-[var(--txt-secondary)]">{p}</span>
                            <div className="min-w-0 flex-1">
                              <div className="h-6 overflow-hidden rounded bg-[var(--bg-layer-1)]">
                                <div
                                  className="h-full rounded"
                                  style={{ width: `${width}%`, minWidth: count ? 8 : 0, maxWidth: '100%', backgroundColor: barColor }}
                                />
                              </div>
                            </div>
                            <span className="w-6 text-right text-sm text-[var(--txt-secondary)]">{count}</span>
                          </div>
                        );
                      })}
                    </div>
              ...
              <section>
                <h2 className="mb-3 text-sm font-semibold text-[var(--txt-primary)]">Work items by state</h2>
                <Card variant="outlined" className="border border-[var(--border-subtle)] bg-white">
                    ...
                    <div className="flex flex-col gap-1.5">
                      {stateCountsForDonut.map((item) => (
                        <div key={item.label} className="flex items-center gap-2 text-sm">
                          ...
                          <span className="text-[var(--txt-secondary)]">{item.label}</span>
                          <span className="text-[var(--txt-primary)]">({item.count})</span>
                        </div>
                      ))}
                    </div>
                  </CardContent>
            ...
            {/* Recent activity */}
            <section>
              <h2 className="mb-3 text-sm font-semibold text-[var(--txt-primary)]">Recent activity</h2>
              <Card variant="outlined" className="border border-[var(--border-subtle)] bg-white">
                <CardContent className="p-0">
                  <ul className="divide-y divide-[var(--border-subtle)]">
                    {recentActivity.length === 0 ? (
                      <li className="px-4 py-6 text-center text-sm text-[var(--txt-tertiary)]">No recent activity</li>
                    ) : (
                      recentActivity.map((act) => {
                        const project = projects.find((p) => p.id === act.projectId);
                        const issue = issues.find((i) => i.id === act.issueId && i.project_id === act.projectId);
                        const issueRef = project && issue ? `${project.identifier ?? ''}-${issue.sequence_id ?? issue.id.slice(-4)}` : '';
                        const issueTitle = issue?.name ?? '';
                        let text: React.ReactNode = '';
                        if (act.type === 'label' && act.labelName) {
                          text = <>You added a new label <span className="rounded bg-[var(--bg-warning-subtle)] px-1.5 py-0.5 text-[var(--txt-warning-primary)]">{act.labelName}</span> to {issueRef} {issueTitle} {formatTimeAgo(act.createdAt)}</>;
                        } else if (act.type === 'cycle' && act.cycleName) {
                          text = <>You added {issueRef} {issueTitle} to the cycle {act.cycleName} {formatTimeAgo(act.createdAt)}</>;
                        } else if (act.type === 'assignee' && act.assigneeName) {
                          text = <>You added a new assignee {act.assigneeName} to {issueRef} {formatTimeAgo(act.createdAt)}</>;
                        } else if (act.type === 'created') {
                          text = <>You created {issueRef} {issueTitle} {formatTimeAgo(act.createdAt)}</>;
                        } else {
                          text = <>Activity {formatTimeAgo(act.createdAt)}</>;
                        }
                        return (
                          <li key={act.id} className="flex gap-3 px-4 py-3">
                            <Avatar name={profileUser?.name ?? '?'} src={profileUser?.avatarUrl} size="sm" className="shrink-0" />
                            <p className="min-w-0 text-sm text-[var(--txt-secondary)]">{text}</p>
                          </li>
                        );
                      })
                    )}
      ...
      <aside className="hidden w-72 shrink-0 lg:flex lg:flex-col">
        <Card variant="outlined" className="flex min-h-0 flex-1 flex-col overflow-hidden border border-[var(--border-subtle)] bg-white">
          ...
          <div className="relative flex min-h-0 flex-1 flex-col px-4 pb-4" style={{ marginTop: '-28px' }}>
            <div className="mb-3 flex justify-center shrink-0">
              <Avatar name={profileUser?.name ?? '?'} src={profileUser?.avatarUrl} size="lg" className="h-14 w-14 border-4 border-white shadow" />
            </div>
            <h3 className="text-center text-base font-semibold text-[var(--txt-primary)] shrink-0">{profileUser?.name ?? '—'}</h3>
            <p className="text-center text-sm text-[var(--txt-tertiary)] shrink-0">({profileUser?.email?.split('@')[0] ?? 'user'})</p>
            <p className="mt-2 text-center text-xs text-[var(--txt-secondary)] shrink-0">Joined on {formatJoinedDate(joinedAt)}</p>
            <p className="text-center text-xs text-[var(--txt-secondary)] shrink-0">Timezone 23:10 UTC</p>
            <div className="mt-4 flex min-h-0 flex-1 flex-col pt-4">
              <p className="mb-2 shrink-0 text-xs font-medium uppercase tracking-wide text-[var(--txt-tertiary)]">Projects</p>
              <ul className="min-h-0 flex-1 overflow-auto">
                {projectStateBreakdown.map(({ project: p, states }, index) => {
                  ...
                  const projectIcon = p.name.toLowerCase().includes('logistics') ? (
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="shrink-0 text-[var(--txt-icon-tertiary)]">
                  ...
                  return (
                    <li key={p.id} className={index > 0 ? 'border-t border-[var(--border-subtle)]' : undefined}>
                      <div className="py-1">
                        <button
                          type="button"
                        ...
                        >
                          <span className="flex min-w-0 items-center gap-2">
                            {projectIcon}
                            <Link to={`${baseUrl}/projects/${p.id}`} onClick={(e) => e.stopPropagation()} className="truncate hover:underline">
                              {p.name}
                            </Link>
                          </span>
                          {p.progress > 0 ? (
                            <span className="shrink-0 rounded px-1.5 py-0.5 text-xs text-[var(--txt-secondary)] bg-[var(--bg-layer-2)]">{p.progress}%</span>
                          ) : (
                            <span className="shrink-0 rounded px-1.5 py-0.5 text-xs text-[var(--txt-danger-primary)] bg-[var(--bg-danger-subtle)]">0%</span>
                          )}
                          <svg
                            width="14"
                            height="14"
                            ...
                            <ul className="space-y-1">
                              {states.map((s) => (
                                <li key={s.id} className="flex items-center gap-2 text-xs">
                                  <span className="h-3 w-3 shrink-0 rounded-sm" style={{ backgroundColor: s.color ?? undefined }} aria-hidden />
                                  <span className="text-[var(--txt-secondary)]">{s.name}</span>
                                  <span className="text-[var(--txt-primary)]">— {s.count} Work item{s.count === 1 ? '' : 's'}</span>
                                </li>
                              ))}
                            </ul>
                          </div>
...
function WorkItemsList({
  ...
  issues: Array<{ id: string; project_id?: string; state_id?: string | null; name: string; sequence_id?: number }>;
  states: StateApiResponse[];
  projects: ProjectApiResponse[];
  baseUrl: string;
  workspaceSlug: string;
}) {
  const getStateName = (stateId: string) => statesList.find((s) => s.id === stateId)?.name ?? stateId;
  ...
  return (
    <div className="space-y-0">
      ...
      <Card variant="outlined" className="overflow-hidden border border-[var(--border-subtle)] bg-white">
        <CardContent className="p-0">
          {issues.length === 0 ? (
            <div className="py-12 text-center text-sm text-[var(--txt-tertiary)]">No work items</div>
          ) : (
            <ul className="divide-y divide-[var(--border-subtle)]">
              {issues.map((issue) => {
                const apiIssue = issue as { id: string; project_id?: string; state_id?: string | null; name: string; sequence_id?: number };
                const project = apiIssue.project_id ? projectsList.find((p) => p.id === apiIssue.project_id) : null;
                const issueRef = project ? `${project.identifier ?? ''}-${apiIssue.sequence_id ?? issue.id.slice(-4)}` : issue.id;
                ...
                return (
                  <li key={issue.id}>
                    ...
                    >
                      ...
                      {assignee && (
                        <Avatar name={assignee.name} src={assignee.avatarUrl} size="sm" className="shrink-0" />
                      )}
                      {cycle && (
...
function ActivityTab({
  ...
  activities: Array<{ id: string; projectId: string; issueId: string; type: string; createdAt: string; labelName?: string; cycleName?: string; assigneeName?: string }>;
  projects: ProjectApiResponse[];
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-sm font-semibold text-[var(--txt-primary)]">Recent activity</h2>
        <button
          type="button"
      ...
      <Card variant="outlined" className="border border-[var(--border-subtle)] bg-white">
        <CardContent className="p-0">
          <ul className="divide-y divide-[var(--border-subtle)]">
            {activities.length === 0 ? (
              <li className="px-4 py-8 text-center text-sm text-[var(--txt-tertiary)]">No recent activity</li>
            ) : (
              activities.map((act) => {
                const project = projectsList.find((p) => p.id === act.projectId);
                const issue = issuesList.find((i) => i.id === act.issueId && i.project_id === act.projectId);
                const issueRef = project && issue ? `${project.identifier ?? ''}-${issue.sequence_id ?? issue.id.slice(-4)}` : '';
                const issueTitle = issue?.name ?? '';
                let icon = (
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                if (act.type === 'label' && act.labelName) {
                  ...
                  text = (
                    <>
                      ...
                      to <strong>{issueRef} {issueTitle}</strong> {formatTimeAgo(act.createdAt)}.
                    </>
                  );
                } else if (act.type === 'cycle' && act.cycleName) {
                  icon = (
                  ...
                  text = (
                    <>
                      You added <strong>{issueRef} {issueTitle}</strong> to the cycle <strong>{act.cycleName}</strong> {formatTimeAgo(act.createdAt)}.
                    </>
                  );
                } else if (act.type === 'assignee' && act.assigneeName) {
                  icon = (
                  ...
                  text = (
                    <>
                      You added a new assignee <strong>{act.assigneeName}</strong> to <strong>{issueRef}</strong> {formatTimeAgo(act.createdAt)}.
                    </>
                  );
                } else if (act.type === 'created') {
                  text = (
                    <>
                      You created <strong>{issueRef} {issueTitle}</strong> {formatTimeAgo(act.createdAt)}.
                    </>
                  );
                } else {
                  text = <>Activity {formatTimeAgo(act.createdAt)}.</>;
