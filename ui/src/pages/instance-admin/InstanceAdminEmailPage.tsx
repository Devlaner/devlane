...
export function InstanceAdminEmailPage() {
  const [email, setEmail] = useState<InstanceEmailSection>({
    host: '', port: '587', sender_email: '', security: 'TLS', username: '', password_set: false,
  });
  ...
  const handleSave = () => {
    ...
    const payload: InstanceEmailSection = { ...email, password: passwordToSend ?? '' };
    instanceSettingsService
  ...
  return (
    <div className="w-full max-w-6xl space-y-6">
      <div>
        <h1 className="text-base font-semibold text-[var(--txt-primary)]">Secure emails from your own instance</h1>
        <p className="mt-0.5 text-xs text-[var(--txt-secondary)]">
          ...
          <span className="text-[var(--txt-danger-primary)]">Misconfigs can lead to email bounces and errors.</span>
        </p>
      </div>
      ...
      <section className="space-y-3">
        <h2 className="text-xs font-semibold uppercase tracking-wide text-[var(--txt-secondary)]">Authentication</h2>
        ...
        <label className="block text-xs font-medium text-[var(--txt-secondary)]">
          ...
          <p className="mt-0.5 text-[11px] text-[var(--txt-tertiary)]">Leave blank to keep existing password.</p>
        </label>
      </section>
      ...
      <div className="flex gap-2">
        ...
        <Button variant="secondary" size="sm" className="text-xs" disabled>Send test email</Button>
      </div>
    </div>
  );
}
