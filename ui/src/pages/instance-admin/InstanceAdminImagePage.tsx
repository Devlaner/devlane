...
export function InstanceAdminImagePage() {
  const [image, setImage] = useState<InstanceImageSection>({ unsplash_access_key_set: false });
  ...
  const handleSave = () => {
    setError('');
    setSaving(true);
    const accessKeyToSend = accessKeyLocal !== undefined ? accessKeyLocal : image.unsplash_access_key;
    const payload: InstanceImageSection = { ...image, unsplash_access_key: accessKeyToSend ?? '' };
    instanceSettingsService
  ...
  return (
    <div className="w-full max-w-6xl space-y-6">
      <div>
        <h1 className="text-base font-semibold text-[var(--txt-primary)]">Third-party image libraries</h1>
      ...
      <section className="space-y-3">
        <label className="block text-xs font-medium text-[var(--txt-secondary)]">
          ...
          <p className="mt-0.5 text-[11px] text-[var(--txt-tertiary)]">
            ...
            <a href="https://unsplash.com/developers/docs/api-reference/access-key" target="_blank" rel="noopener noreferrer" className="text-[var(--txt-accent-primary)] underline hover:no-underline">
              Learn more.
            </a>
          </p>
        </label>
