...
export const instanceService = {
  async getSetupStatus(): Promise<InstanceSetupStatusResponse> {
    const { data } = await apiClient.get<InstanceSetupStatusResponse>('/api/instance/setup-status/');
    return data;
  },
...
export const instanceSettingsService = {
  ...
  async updateSection(key: string, value: InstanceSettingSectionValue): Promise<{ key: string; value: InstanceSettingSectionValue }> {
    const { data } = await apiClient.patch<{ key: string; value: InstanceSettingSectionValue }>(
      `/api/instance/settings/${encodeURIComponent(key)}`,
      { value }
    );
    return data;
  },
